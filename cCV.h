...
public:
	cCV();
	cCV(string nume, string prenume, string nrtel, string categorie, string liceu, string experienta, string domeniiActivare, string alteInformatii) :nume(nume), prenume(prenume), nrtel(nrtel), categorie(categorie), liceu(liceu), experienta(experienta), domeniiActivare(domeniiActivare), alteInformatii(alteInformatii) {}
	...
	void setStudii(string std) { this->studiiSuperioare = std; }
	virtual  ~cCV() {};
};
